...
public class NotificacaoPedidoCanceladoConsumer {
    ...
    public void onPedidoCancelado(PedidoCanceladoEvent event) {
        ...
        if (pedido == null) {
            log.error("Pedido {} não encontrado para enviar notificação de cancelamento", event.getPedidoId());
            return;
        }
        ...
        if (usuario == null) {
            log.error("Usuário {} não encontrado para enviar notificação de cancelamento", pedido.getCompradorId());
            return;
        }
        ...
        enviarEmailNotificacao.enviar(
                usuario.getEmail(),
                "Pedido Cancelado #" + pedido.getId().toString().substring(0, 8),
                "pedido-cancelado",
                variables
        );
    }
